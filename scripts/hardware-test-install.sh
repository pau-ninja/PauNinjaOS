...
if [ ! -e /System/Library/CoreServices/SystemVersion.plist ]; then
  echo "Hardware-test installation requires a full macOS environment." >&2
  exit 1
fi
export PATH="/usr/bin:/bin:/usr/sbin:/sbin:$PATH"
for tool in caffeinate cp curl cut grep id mkdir mktemp plutil python3 rm shasum stat sysctl tar; do
...
case "$installable" in
  false|0) ;;
  *) echo "This helper accepts only an unpromoted source-buildable release." >&2; exit 1 ;;
esac
if [ "$status" != SOURCE_BUILDABLE ]; then
  echo "This helper accepts only an unpromoted source-buildable release." >&2
  exit 1
fi

machine=$(/usr/sbin/sysctl -n hw.model)
python3 "$script_dir/release.py" check-test-model "$machine"
...
curl --fail --proto '=https' --proto-redir '=https' --no-progress-meter -L --max-filesize "$installer_size" -o "$archive" "https://cdn.asahilinux.org/installer/$archive"
if [ "$(stat -f %z "$archive")" != "$installer_size" ] ||
   [ "$(shasum -a 256 "$archive" | cut -d ' ' -f 1)" != "$installer_sha" ]; then
